...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'workforce')
...
for (const vp of VIEWPORTS) {
  test.describe(`Workforce E2E @ ${vp.name}`, () => {
    test.use({ viewport: { width: vp.width, height: vp.height }, storageState: { cookies: [], origins: [] } })
    test('workforce workflow', async ({ page }) => {
      await signIn(page, USER, PASS)
      await page.goto('#/workforce')
      await waitLoad(page)
      await settle(page, 800)
      await shot(page, vp.name, 1, 'roster')

      const tabs = [
        { rx: /^time tracking/i, name: 'time-tracking' },
        { rx: /^credentials/i, name: 'credentials' },
        { rx: /^forecast/i, name: 'forecast' },
        { rx: /^productivity/i, name: 'productivity' },
      ]
      ...
      if (await tryClick(page, /add worker/i)) {
        await settle(page, 500)
        await shot(page, vp.name, 6, 'add-worker')
