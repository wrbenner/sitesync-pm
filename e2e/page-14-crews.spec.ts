...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'crews')
...
for (const vp of VIEWPORTS) {
  test.describe(`Crews E2E @ ${vp.name}`, () => {
    test.use({ viewport: { width: vp.width, height: vp.height }, storageState: { cookies: [], origins: [] } })
    test('crews workflow', async ({ page }) => {
      await signIn(page, USER, PASS)
      await page.goto('#/crews')
      await waitLoad(page)
      await settle(page, 800)
      await shot(page, vp.name, 1, 'cards')

      if (await tryClick(page, /^map$/i)) { await settle(page, 600); await shot(page, vp.name, 2, 'map') }
      if (await tryClick(page, /^performance$/i)) { await settle(page, 500); await shot(page, vp.name, 3, 'performance') }
      if (await tryClick(page, /add crew/i)) {
        await settle(page, 500)
        await shot(page, vp.name, 4, 'add-crew')
