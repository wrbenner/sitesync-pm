...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'audit-trail')
...
for (const vp of VIEWPORTS) {
  test.describe(`Audit Trail E2E @ ${vp.name}`, () => {
    test.use({ viewport: { width: vp.width, height: vp.height }, storageState: { cookies: [], origins: [] } })
    test('audit-trail workflow', async ({ page }) => {
      await signIn(page, USER, PASS)
      await page.goto('#/audit-trail', { waitUntil: 'domcontentloaded' })
      await waitLoad(page)
