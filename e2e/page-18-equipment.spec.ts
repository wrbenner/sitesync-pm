...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'equipment')
  ...
  test.describe(`Equipment E2E @ ${vp.name}`, () => {
    ...
    test('equipment workflow', async ({ page }) => {
      ...
      if (await tryClick(page, /add equipment/i)) {
        ...
        await shot(page, vp.name, 2, 'add-modal')
