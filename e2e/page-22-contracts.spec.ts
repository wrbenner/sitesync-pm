...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'contracts')
...
for (const vp of VIEWPORTS) {
  test.describe(`Contracts E2E @ ${vp.name}`, () => {
    test.use({ viewport: { width: vp.width, height: vp.height }, storageState: { cookies: [], origins: [] } })
    test('contracts workflow', async ({ page }) => {
      await signIn(page, USER, PASS)
      await page.goto('#/contracts')
      await waitLoad(page)
      await settle(page, 800)
      await shot(page, vp.name, 1, 'contracts-tab')

      const tabs = [
        { rx: /^vendors$/i, name: 'vendors' },
        { rx: /^insurance$/i, name: 'insurance' },
        { rx: /^change orders$/i, name: 'change-orders' },
      ]
