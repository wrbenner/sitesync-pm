...
const OUT_DIR = path.resolve(__dirname, '..', 'polish-review', 'pages', 'time-tracking')
...
for (const vp of VIEWPORTS) {
  test.describe(`Time Tracking E2E @ ${vp.name}`, () => {
    ...
    test('time-tracking workflow', async ({ page }) => {
      ...
      await shot(page, vp.name, 1, 'timesheet')

      const tabs = [
        { rx: /^certified payroll/i, name: 'certified-payroll' },
        { rx: /^t&m tickets/i, name: 'tm-tickets' },
        { rx: /^rates/i, name: 'rates' },
        { rx: /^payroll export/i, name: 'payroll-export' },
      ]
