...
1. **Typecheck is load-bearing — green or you don't ship.** As of 2026-05-03 the codebase is undergoing a campaign to clear the historical 4339-error baseline; the destination state is **zero errors**, enforced by CI (`test.yml` runs `tsc --noEmit` on every PR + push). Do not introduce new errors. Run `npm run typecheck` after every batch of changes. The bash sandbox in our cowork sessions has a 45-second window — full tsc cold-start exceeds it. Use the longer window your harness gives you. If a fresh schema migration was just applied, run `npm run db-types:write` and commit the regenerated `database.ts` in the same PR; `db-types:check` will compare locally against the live schema.
