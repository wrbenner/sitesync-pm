...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch sheet numbers for both drawings so the auto-drafted RFI cites them.
    const { data: drawingRows } = await supabase
      .from('drawings')
      .select('id, sheet_number, title')
      .in('id', [pair.arch_drawing_id, pair.struct_drawing_id].filter(Boolean))
    const archSheet = drawingRows?.find((d) => d.id === pair.arch_drawing_id)
    const structSheet = drawingRows?.find((d) => d.id === pair.struct_drawing_id)
    const archSheetLabel = archSheet?.sheet_number || archSheet?.title || 'Architectural'
    const structSheetLabel = structSheet?.sheet_number || structSheet?.title || 'Structural'
    ...
    try {
      ...
      if (discrepancyInserts.length > 0) {
        const { data: insertData, error: insertErr } = await supabase
          ...
          .select('id, severity, description, arch_dimension, struct_dimension')
        if (insertErr) {
          throw new HttpError(500, `Failed to insert discrepancies: ${insertErr.message}`)
        }
        inserted = (insertData ?? []) as typeof inserted
      }

      // Auto-draft RFIs for high-severity discrepancies, with full context.
      for (const d of inserted) {
        if (d.severity !== 'high') continue

        const archInches = parseArchitecturalDim(d.arch_dimension) ?? null
        const structInches = parseArchitecturalDim(d.struct_dimension) ?? null
        const diffInches = archInches !== null && structInches !== null
          ? Math.abs(archInches - structInches)
          : null

        const priority = d.severity === 'high' && diffInches !== null && diffInches >= MAJOR_THRESHOLD_INCHES
          ? 'urgent'
          : 'high'

        // Suggested resolution tailored to the discrepancy type.
        const descLower = d.description.toLowerCase()
        let suggestedResolution = 'Confirm the governing dimension and update the drawing set accordingly.'
        if (descLower.includes('wall')) {
          suggestedResolution = 'Confirm wall location with the architect and coordinate with structural before pouring foundations or framing.'
        } else if (descLower.includes('column') || descLower.includes('grid')) {
          suggestedResolution = 'Verify grid/column coordinates with the structural engineer; update the arch set if struct governs, or issue an ASI if arch governs.'
        } else if (descLower.includes('slab') || descLower.includes('floor')) {
          suggestedResolution = 'Review slab elevation/thickness with structural; update as-builts and coordinate with all trades before pouring.'
        } else if (descLower.includes('opening') || descLower.includes('door') || descLower.includes('window')) {
          suggestedResolution = 'Confirm rough opening dimensions with the architect; update structural lintel/header sizing if required.'
        }

        const rfiTitle = `AI: ${d.description.slice(0, 120)}`
        const rfiDescription = [
          `⚠️ Auto-generated by SiteSync AI drawing intelligence.`,
          ``,
          `**Discrepancy:** ${d.description}`,
          ``,
          `**Sheets in conflict:**`,
          `  • Architectural: ${archSheetLabel}`,
          `  • Structural:    ${structSheetLabel}`,
          ``,
          `**Dimensions in conflict:**`,
          `  • Architectural dimension: ${d.arch_dimension ?? '(unreadable)'}`,
          `  • Structural dimension:    ${d.struct_dimension ?? '(unreadable)'}`,
          diffInches !== null ? `  • Delta: ${diffInches.toFixed(2)}" (severity: ${d.severity})` : `  • Severity: ${d.severity}`,
          ``,
          `**Suggested resolution:** ${suggestedResolution}`,
          ``,
          `Review the clash-detection panel for arch vs struct overlay and respond via RFI workflow.`,
        ].join('\n')
