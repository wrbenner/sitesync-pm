...
CREATE INDEX idx_notification_queue_status ON notification_queue(status, created_at) WHERE status = 'pending';
CREATE INDEX idx_notification_queue_project ON notification_queue(project_id);
CREATE INDEX idx_notification_queue_recipient ON notification_queue(recipient_user_id);

ALTER TABLE notification_queue ENABLE ROW LEVEL SECURITY;

CREATE POLICY 'Project members can view own notifications' ON notification_queue
