  ...
  return `
    <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
      <div style="border-bottom: 2px solid #F47820; padding-bottom: 16px; margin-bottom: 24px;">
        ...
        <p style="color: #5C5550; margin: 4px 0 0; font-size: 13px;">${rfi.projectName}</p>
      </div>
      <h3 style="color: #1A1613; margin: 0 0 8px;">RFI ${rfi.number} Response</h3>
      <p style="color: #5C5550; font-size: 14px; margin: 0 0 16px;">${rfi.title}</p>
      <div style="background: #F8F9FA; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
        <p style="color: #5C5550; font-size: 12px; margin: 0 0 4px;">Response from ${rfi.respondedBy}:</p>
        <p style="color: #1A1613; font-size: 14px; margin: 0; line-height: 1.6;">${rfi.response}</p>
      </div>
...
function submittalTransmittalHtml(submittal: { number: string; title: string; status: string; specSection: string; projectName: string }): string {
  const statusColor = submittal.status === 'approved' ? '#2D8A6E' : submittal.status === 'rejected' ? '#C93B3B' : '#C4850C'
  return `
    <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
      <div style="border-bottom: 2px solid #F47820; padding-bottom: 16px; margin-bottom: 24px;">
        <h2 style="color: #1A1613; margin: 0;">SiteSync PM</h2>
        <p style="color: #5C5550; margin: 4px 0 0; font-size: 13px;">${submittal.projectName}</p>
      </div>
      <h3 style="color: #1A1613; margin: 0 0 8px;">Submittal Transmittal</h3>
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 16px;">
        <tr><td style="padding: 8px; color: #5C5550; font-size: 13px; border-bottom: 1px solid #E5E1DC;">Submittal #</td><td style="padding: 8px; color: #1A1613; font-size: 13px; border-bottom: 1px solid #E5E1DC; font-weight: 600;">${submittal.number}</td></tr>
        <tr><td style="padding: 8px; color: #5C5550; font-size: 13px; border-bottom: 1px solid #E5E1DC;">Title</td><td style="padding: 8px; color: #1A1613; font-size: 13px; border-bottom: 1px solid #E5E1DC;">${submittal.title}</td></tr>
        <tr><td style="padding: 8px; color: #5C5550; font-size: 13px; border-bottom: 1px solid #E5E1DC;">Spec Section</td><td style="padding: 8px; color: #1A1613; font-size: 13px; border-bottom: 1px solid #E5E1DC;">${submittal.specSection}</td></tr>
        <tr><td style="padding: 8px; color: #5C5550; font-size: 13px;">Status</td><td style="padding: 8px;"><span style="color: ${statusColor}; font-weight: 600; font-size: 13px;">${submittal.status.replace(/_/g, ' ').toUpperCase()}</span></td></tr>
      </table>
      <p style="color: #9A9490; font-size: 12px; margin: 24px 0 0; border-top: 1px solid #E5E1DC; padding-top: 12px;">
        Sent via SiteSync PM. Do not reply to this email.
      </p>
...
function dailyLogSummaryHtml(log: { date: string; workers: number; manHours: number; incidents: number; weather: string; summary: string; projectName: string }): string {
  return `
    <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
      <div style="border-bottom: 2px solid #F47820; padding-bottom: 16px; margin-bottom: 24px;">
        <h2 style="color: #1A1613; margin: 0;">SiteSync PM</h2>
        <p style="color: #5C5550; margin: 4px 0 0; font-size: 13px;">${log.projectName}</p>
      </div>
      <h3 style="color: #1A1613; margin: 0 0 16px;">Daily Log Summary: ${log.date}</h3>
      <div style="display: flex; gap: 16px; margin-bottom: 20px;">
        <div style="flex: 1; background: #F8F9FA; border-radius: 8px; padding: 12px; text-align: center;">
          <p style="font-size: 24px; font-weight: 700; color: #1A1613; margin: 0;">${log.workers}</p>
          <p style="font-size: 12px; color: #5C5550; margin: 4px 0 0;">Workers</p>
        </div>
        <div style="flex: 1; background: #F8F9FA; border-radius: 8px; padding: 12px; text-align: center;">
          <p style="font-size: 24px; font-weight: 700; color: #1A1613; margin: 0;">${log.manHours}</p>
          <p style="font-size: 12px; color: #5C5550; margin: 4px 0 0;">Man Hours</p>
        </div>
        <div style="flex: 1; background: ${log.incidents > 0 ? '#FEF2F2' : '#F8F9FA'}; border-radius: 8px; padding: 12px; text-align: center;">
          <p style="font-size: 24px; font-weight: 700; color: ${log.incidents > 0 ? '#C93B3B' : '#2D8A6E'}; margin: 0;">${log.incidents}</p>
          <p style="font-size: 12px; color: #5C5550; margin: 4px 0 0;">Incidents</p>
        </div>
      </div>
      ${log.weather ? `<p style="color: #5C5550; font-size: 13px; margin: 0 0 12px;">Weather: ${log.weather}</p>` : ''}
      ${log.summary ? `<p style="color: #1A1613; font-size: 14px; margin: 0; line-height: 1.6;">${log.summary}</p>` : ''}
      <p style="color: #9A9490; font-size: 12px; margin: 24px 0 0; border-top: 1px solid #E5E1DC; padding-top: 12px;">
        Sent via SiteSync PM. Do not reply to this email.
      </p>
    </div>
