  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SearchResult[]> => {
      ...
      if (searchTypes.includes('document')) {
        ...
        if (data) {
          results.push(
            ...(data as any[]).map((d: any) => ({ // type-safe-ok (dynamic full-text-search fan-out — tables share shape but differ in DB types)
              id: d.id,
      ...
      if (searchTypes.includes('file')) {
        ...
        if (data) {
          results.push(
            ...(data as any[]).map((d: any) => ({ // type-safe-ok (dynamic full-text-search fan-out)
              id: d.id,
              type: 'file' as const,
              title: d.name,
              description: d.description,
