...
import { insertActivityEvent } from '../../lib/activityEvent'
...
export function useCreateIncident() {
  ...
  return useMutation({
    ...
    onSuccess: (result: { data: Record<string, unknown>; projectId: string }) => {
      ...
      const entityId = result.data?.id as string | undefined
      const entityTitle = (result.data?.description as string | undefined) || 'Safety Incident'
      insertActivityEvent(result.projectId, 'safety_incident', entityId, entityTitle, 'created').catch(() => {})
    },
