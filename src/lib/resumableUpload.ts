...
export function uploadResumable({
  ...
  // The Promise<ResumableUploadResult> resolves once the TUS upload finishes,
  // errors, or the auth/setup phase fails. We can't use an async executor
  // (eslint: no-async-promise-executor — rejections from a thrown promise body
  // are silently swallowed instead of rejecting the outer Promise). Instead,
  // a sync executor captures resolve/reject and an async setup() wires them up.
