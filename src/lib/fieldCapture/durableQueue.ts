...
/** Random UUID — target runtimes (Node 22, modern browsers) always have crypto.randomUUID. */
...
export function nextDelayMs(attempts: number): number {
  ...
  const jitter = (globalThis.crypto.getRandomValues(new Uint8Array(1))[0] / 255) * capped * 0.2  // ±20%
