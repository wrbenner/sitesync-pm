...
/** Random UUID using the Web Crypto API (supported in all targeted runtimes). */
...
export function nextDelayMs(attempts: number): number {
  ...
  const jitterRnd = crypto.getRandomValues(new Uint32Array(1))[0] / 0xffffffff
  const jitter = jitterRnd * capped * 0.2  // ±20%
