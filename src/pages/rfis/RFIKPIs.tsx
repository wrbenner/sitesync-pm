...
const MiniSparkline: React.FC<{ data: number[]; color: string; width?: number; height?: number }> = ({
  ...
  const id = `rfi-spark-${color.replace(/[^a-z0-9]/gi, '')}`
  return (
...
const ResolutionRing: React.FC<{ pct: number; size?: number; stroke?: number }> = ({
...
export const RFIKPIs: React.FC<RFIKPIsProps> = React.memo(({
  ...
  const resolutionPct = totalRfis > 0 ? Math.round((closedCount / totalRfis) * 100) : 0

  const costSparkData = React.useMemo(() => {
    const base = totalCostImpact
    return [base * 0.4, base * 0.5, base * 0.65, base * 0.7, base * 0.8, base * 0.9, base]
  }, [totalCostImpact])
  ...
  return (
    <>
      <style>{`
        .rfi-kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 12px; margin-bottom: 20px; }
        @media (max-width: 1100px) { .rfi-kpi-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 580px) { .rfi-kpi-grid { grid-template-columns: 1fr; } }
      `}</style>
