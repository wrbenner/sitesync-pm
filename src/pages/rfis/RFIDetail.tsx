...
type ResponseWithExtras = RFIResponse & { company?: string | null }
...
export function RFIDetail() {
  ...
  useEffect(() => {
    if (lastViewedKey) {
      try { localStorage.setItem(lastViewedKey, new Date().toISOString()) } catch (_e) { /* storage unavailable */ }
    }
  }, [lastViewedKey])

  const rfi = rfiData as (RFIWithExtras & { responses?: ResponseWithExtras[] }) | undefined
