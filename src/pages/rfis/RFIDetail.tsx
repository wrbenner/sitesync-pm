...
// These fields may arrive via JOIN/select expansion but are not in the generated Row types.
interface ExtendedRFI extends RFI {
  from_company?: string | null
  question?: string | null
}
interface ExtendedRFIResponse extends RFIResponse {
  company?: string | null
}
...
export function RFIDetail() {
  ...
  const rfi = rfiData as (ExtendedRFI & { responses?: ExtendedRFIResponse[] }) | undefined
        ...
        {currentStatus !== 'void' && (
          <div style={{
            ...
            <WorkflowTimeline
              ...
              completedStates={
                (() => {
                  const ordered = ['draft', 'open', 'under_review', 'answered', 'closed']
                  const idx = ordered.indexOf(currentStatus)
                  return ordered.slice(0, Math.max(0, idx))
                })()
              }
            />
          </div>
