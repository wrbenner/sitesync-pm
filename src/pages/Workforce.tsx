...
export const Workforce: React.FC = () => {
  ...
  const activeToday = members?.filter((m: unknown) => (m as Record<string, unknown>).status === 'active').length || 0
  const totalRegularHrs = timeEntries?.reduce((s: number, e: unknown) => s + (Number((e as Record<string, unknown>).regular_hours) || 0), 0) || 0
  const totalOTHrs = timeEntries?.reduce((s: number, e: unknown) => s + (Number((e as Record<string, unknown>).overtime_hours) || 0), 0) || 0
