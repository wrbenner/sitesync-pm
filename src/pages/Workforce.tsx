...
export const Workforce: React.FC = () => {
  ...
  const totalRegularHrs = timeEntries?.reduce((s, e) => s + ((e as { regular_hours?: number | null }).regular_hours || 0), 0) || 0
  const totalOTHrs = timeEntries?.reduce((s, e) => s + ((e as { overtime_hours?: number | null }).overtime_hours || 0), 0) || 0
  ...
  const timeColumnsWithActions = [
    ...
    timeCol.display({
      ...
      header: '',
      cell: (info) => {
        const row = info.row.original as { id: string; approved?: boolean | null; project_id?: string }
