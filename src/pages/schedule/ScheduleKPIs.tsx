...
function MiniSparkline({ value, color, max = 100 }: { value: number; color: string; max?: number }) {
  // Deterministic linear ramp from baseline → current value. Real
  // historical points will populate this once schedule snapshots are
  // wired; until then the sparkline must be stable between renders.
  const points = React.useMemo(() => {
    const baseline = Math.max(0, value - 15);
    const pts = [0, 1, 2, 3, 4].map(i => baseline + ((value - baseline) * i) / 4);
    return pts.map(v => Math.min(max, Math.max(0, v)));
  }, [value, max]);
