...
const DailySummaryPage: React.FC = () => {
  ...
  const dailyLog = useMemo(() => {
    ...
    return dailyLogData.data.find((log) => {
      const row = log as { log_date?: string; date?: string };
      const logDate = row.log_date ?? row.date ?? '';
  ...
  const summaryProps: AIDailySummaryProps = useMemo(() => {
    ...
    if (totalWorkers > 0) {
      ...
      if (log?.manpower && Array.isArray(log.manpower)) {
        type ManpowerRow = { trade?: string; category?: string; count?: number; workers?: number };
        for (const m of log.manpower as ManpowerRow[]) {
