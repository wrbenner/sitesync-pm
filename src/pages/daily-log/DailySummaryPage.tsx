    ...
    if (totalWorkers > 0) {
      ...
      if (log?.manpower && Array.isArray(log.manpower)) {
        for (const m of log.manpower) {
          const trade = ((m as Record<string, unknown>).trade ?? (m as Record<string, unknown>).category ?? 'General') as string;
          byTrade[trade] = (byTrade[trade] ?? 0) + Number((m as Record<string, unknown>).count ?? (m as Record<string, unknown>).workers ?? 1);
        }
      }
