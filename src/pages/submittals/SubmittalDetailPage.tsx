...
import { useSubmittal, useSubmittalReviewers, type Submittal } from '../../hooks/queries/submittals'
...
export function SubmittalDetailPage() {
  ...
  const sub = submittal ?? ({} as Submittal)
  ...
  useEffect(() => {
    ...
    const attachments = ((submittal as Submittal & { attachments?: unknown[] }).attachments || [])
  ...
  const handleDocumentUpload = useCallback(async (file: File) => {
    ...
    const currentAttachments = ((submittal as Submittal & { attachments?: unknown[] }).attachments || [])
    const newAttachment = {
      path: storagePath,
      name: file.name,
      size: file.size,
