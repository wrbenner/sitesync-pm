...
export function SubmittalDetailPage() {
  ...
  const { data: submittalRaw, isLoading, error } = useSubmittal(submittalId)
  // The DB select includes attachments column not yet reflected in generated types
  type SubmittalWithAttachments = typeof submittalRaw & { attachments?: unknown[] } | undefined
  const submittal = submittalRaw as SubmittalWithAttachments
  ...
  useEffect(() => {
    ...
    const attachments: unknown[] = submittal?.attachments ?? []
  ...
  const handleDocumentUpload = useCallback(async (file: File) => {
    ...
    const currentAttachments: unknown[] = submittal.attachments ?? []
    const newAttachment = {
      path: storagePath,
      name: file.name,
      size: file.size,
      ...
      <PageContainer>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center', padding: '80px 0' }}>
          ...
          <p style={{ color: colors.textTertiary, margin: '0 0 20px', fontSize: typography.fontSize.body }}>
            {(error as { message?: string })?.message || 'This submittal may have been deleted or you don\'t have access.'}
          </p>
