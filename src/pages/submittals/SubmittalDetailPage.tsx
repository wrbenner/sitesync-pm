...
export function SubmittalDetailPage() {
  ...
  // The submittal row is dynamic: legacy statuses + ad-hoc attachment records
  // make the canonical Submittal type incomplete here. Narrow at the read site
  // with a typed shape rather than reaching for `any`.
  type SubmittalLike = {
    id: string
    status?: string
    attachments?: unknown[]
  }
  const subRow = submittal as SubmittalLike | null
  ...
  const rawStatus = (subRow?.status as string) || 'draft'
  ...
  useEffect(() => {
    ...
    const attachments = (subRow?.attachments || []) as unknown[]
  ...
  const handleDocumentUpload = useCallback(async (file: File) => {
    ...
    const storagePath = `submittals/${projectId}/${subRow?.id ?? ''}/${Date.now()}_${file.name}`
    ...
    const currentAttachments = (subRow?.attachments || []) as unknown[]
    const newAttachment = {
      path: storagePath,
      name: file.name,
      size: file.size,
      ...
      <PageContainer>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center', padding: '80px 0' }}>
          ...
          <p style={{ color: colors.textTertiary, margin: '0 0 20px', fontSize: typography.fontSize.body }}>
            {(error as { message?: string } | null)?.message || 'This submittal may have been deleted or you don\'t have access.'}
          </p>
