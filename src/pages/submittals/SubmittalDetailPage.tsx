...
export function SubmittalDetailPage() {
  ...
  const rawStatus = ((submittal as Record<string, unknown>)?.status as string) || 'draft'
  ...
  const sub = (submittal as Record<string, unknown>) || {}
  ...
  const handleDocumentUpload = useCallback(async (file: File) => {
    ...
    const storagePath = `submittals/${projectId}/${sub.id as string}/${Date.now()}_${file.name}`
    ...
    const currentAttachments = (sub.attachments || []) as unknown[]
    const newAttachment = {
      path: storagePath,
      name: file.name,
      size: file.size,
      ...
      <PageContainer>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center', padding: '80px 0' }}>
          ...
          <p style={{ color: colors.textTertiary, margin: '0 0 20px', fontSize: typography.fontSize.body }}>
            {(error instanceof Error ? error.message : 'This submittal may have been deleted or you don\'t have access.')}
          </p>
