...
beforeEach(() => {
  ...
  h.setResult({ data: { id: 'dc1', ...validContact }, error: null })
})
...
describe('useCreateDirectoryContact', () => {
  ...
  it('inserts and invalidates', async () => {
    ...
    expect(h.supabase.from).toHaveBeenCalledWith('directory_contacts')
    await waitFor(() =>
      expect(h.posthogCapture).toHaveBeenCalledWith('directory_contact_created', expect.any(Object)),
    )
  })
})
