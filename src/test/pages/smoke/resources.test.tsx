...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Resources.tsx')

describe('Resources smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Resources = …                        (named const arrow)
    //   export function Resources(…)                      (named function)
    //   export { Resources } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Resources\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bResources\\b').test(src)
