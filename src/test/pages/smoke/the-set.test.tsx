...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/set/index.tsx')

describe('The Set smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Set = …                        (named const arrow)
    //   export function Set(…)                      (named function)
    //   export { Set } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Set\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSet\\b').test(src)
