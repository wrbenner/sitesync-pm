...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/ChangeOrders.tsx')

describe('Change Orders smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const ChangeOrders = …                        (named const arrow)
    //   export function ChangeOrders(…)                      (named function)
    //   export { ChangeOrders } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+ChangeOrders\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bChangeOrders\\b').test(src)
