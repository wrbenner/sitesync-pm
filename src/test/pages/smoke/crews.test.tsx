...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Crews.tsx')

describe('Crews smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Crews = …                        (named const arrow)
    //   export function Crews(…)                      (named function)
    //   export { Crews } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Crews\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCrews\\b').test(src)
