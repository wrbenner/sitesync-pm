...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Permits.tsx')

describe('Permits smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Permits = …                        (named const arrow)
    //   export function Permits(…)                      (named function)
    //   export { Permits } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Permits\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPermits\\b').test(src)
