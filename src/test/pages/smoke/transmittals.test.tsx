...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Transmittals.tsx')

describe('Transmittals smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Transmittals = …                        (named const arrow)
    //   export function Transmittals(…)                      (named function)
    //   export { Transmittals } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Transmittals\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bTransmittals\\b').test(src)
