...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/ledger/index.tsx')

describe('The Ledger smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Ledger = …                        (named const arrow)
    //   export function Ledger(…)                      (named function)
    //   export { Ledger } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Ledger\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bLedger\\b').test(src)
