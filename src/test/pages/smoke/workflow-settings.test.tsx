...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Settings/WorkflowSettings.tsx')

describe('Workflow Settings smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const WorkflowSettings = …                        (named const arrow)
    //   export function WorkflowSettings(…)                      (named function)
    //   export { WorkflowSettings } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+WorkflowSettings\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bWorkflowSettings\\b').test(src)
