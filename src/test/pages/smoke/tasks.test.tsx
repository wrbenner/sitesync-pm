...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Tasks.tsx')
...
describe('Tasks smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Tasks = …                        (named const arrow)
    //   export function Tasks(…)                      (named function)
    //   export { Tasks } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Tasks\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bTasks\\b').test(src)
