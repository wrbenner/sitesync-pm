...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/crew/index.tsx')

describe('The Crew smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Crew = …                        (named const arrow)
    //   export function Crew(…)                      (named function)
    //   export { Crew } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Crew\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCrew\\b').test(src)
