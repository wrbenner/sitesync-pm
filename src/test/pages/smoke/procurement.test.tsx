...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Procurement.tsx')

describe('Procurement smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Procurement = …                        (named const arrow)
    //   export function Procurement(…)                      (named function)
    //   export { Procurement } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Procurement\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bProcurement\\b').test(src)
