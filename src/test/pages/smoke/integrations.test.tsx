...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Integrations.tsx')
...
describe('Integrations smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Integrations = …                        (named const arrow)
    //   export function Integrations(…)                      (named function)
    //   export { Integrations } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Integrations\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bIntegrations\\b').test(src)
