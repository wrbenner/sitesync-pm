...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/safety/index.tsx')

describe('Safety smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Safety = …                        (named const arrow)
    //   export function Safety(…)                      (named function)
    //   export { Safety } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Safety\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSafety\\b').test(src)
