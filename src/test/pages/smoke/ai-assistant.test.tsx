...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/AIAssistant.tsx')
...
describe('AI Assistant smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const AIAssistant = …                        (named const arrow)
    //   export function AIAssistant(…)                      (named function)
    //   export { AIAssistant } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+AIAssistant\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bAIAssistant\\b').test(src)
