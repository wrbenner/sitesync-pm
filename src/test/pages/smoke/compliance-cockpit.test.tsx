...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/compliance/index.tsx')

describe('Compliance Cockpit smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Compliance = …                        (named const arrow)
    //   export function Compliance(…)                      (named function)
    //   export { Compliance } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Compliance\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCompliance\\b').test(src)
