...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Closeout.tsx')

describe('Closeout smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Closeout = …                        (named const arrow)
    //   export function Closeout(…)                      (named function)
    //   export { Closeout } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Closeout\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCloseout\\b').test(src)
