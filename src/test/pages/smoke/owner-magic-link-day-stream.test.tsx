...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/components/MagicLinkOwnerRoute.tsx')

describe('Owner Magic-Link Day Stream smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const MagicLinkOwnerRoute = …                        (named const arrow)
    //   export function MagicLinkOwnerRoute(…)                      (named function)
    //   export { MagicLinkOwnerRoute } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+MagicLinkOwnerRoute\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bMagicLinkOwnerRoute\\b').test(src)
