...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Settings/Billing.tsx')
...
describe('Billing smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Billing = …                        (named const arrow)
    //   export function Billing(…)                      (named function)
    //   export { Billing } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Billing\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bBilling\\b').test(src)
