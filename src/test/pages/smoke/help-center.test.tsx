...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/help/HelpIndex.tsx')

describe('Help Center smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const HelpIndex = …                        (named const arrow)
    //   export function HelpIndex(…)                      (named function)
    //   export { HelpIndex } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+HelpIndex\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bHelpIndex\\b').test(src)
