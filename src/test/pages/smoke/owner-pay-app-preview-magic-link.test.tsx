...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/share/OwnerPayAppPreview.tsx')

describe('Owner Pay-App Preview (magic link) smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const OwnerPayAppPreview = …                        (named const arrow)
    //   export function OwnerPayAppPreview(…)                      (named function)
    //   export { OwnerPayAppPreview } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+OwnerPayAppPreview\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bOwnerPayAppPreview\\b').test(src)
