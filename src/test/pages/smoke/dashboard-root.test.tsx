...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/dashboard/index.tsx')

describe('Dashboard (root) smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Dashboard = …                        (named const arrow)
    //   export function Dashboard(…)                      (named function)
    //   export { Dashboard } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Dashboard\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bDashboard\\b').test(src)
