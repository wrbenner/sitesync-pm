...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Activity.tsx')

describe('Activity Feed smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Activity = …                        (named const arrow)
    //   export function Activity(…)                      (named function)
    //   export { Activity } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Activity\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bActivity\\b').test(src)
