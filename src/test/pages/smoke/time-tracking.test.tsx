...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/TimeTracking.tsx')
...
describe('Time Tracking smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const TimeTracking = …                        (named const arrow)
    //   export function TimeTracking(…)                      (named function)
    //   export { TimeTracking } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+TimeTracking\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bTimeTracking\\b').test(src)
