...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/OwnerPortal.tsx')
...
describe('Owner Portal smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const OwnerPortal = …                        (named const arrow)
    //   export function OwnerPortal(…)                      (named function)
    //   export { OwnerPortal } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+OwnerPortal\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bOwnerPortal\\b').test(src)
