...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Meetings.tsx')

describe('Meetings smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Meetings = …                        (named const arrow)
    //   export function Meetings(…)                      (named function)
    //   export { Meetings } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Meetings\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bMeetings\\b').test(src)
