...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/procore-import/index.tsx')

describe('Procore Import smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const ProcoreImport = …                        (named const arrow)
    //   export function ProcoreImport(…)                      (named function)
    //   export { ProcoreImport } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+ProcoreImport\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bProcoreImport\\b').test(src)
