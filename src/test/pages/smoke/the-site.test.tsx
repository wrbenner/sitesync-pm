...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/site/index.tsx')

describe('The Site smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Site = …                        (named const arrow)
    //   export function Site(…)                      (named function)
    //   export { Site } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Site\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSite\\b').test(src)
