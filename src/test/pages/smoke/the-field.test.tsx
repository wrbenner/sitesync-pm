...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/field/index.tsx')

describe('The Field smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Field = …                        (named const arrow)
    //   export function Field(…)                      (named function)
    //   export { Field } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Field\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bField\\b').test(src)
