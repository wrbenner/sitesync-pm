...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/project-templates/index.tsx')

describe('Project Templates smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const ProjectTemplates = …                        (named const arrow)
    //   export function ProjectTemplates(…)                      (named function)
    //   export { ProjectTemplates } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+ProjectTemplates\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bProjectTemplates\\b').test(src)
