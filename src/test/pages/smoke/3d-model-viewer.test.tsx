...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/bim/BIMViewerPage.tsx')

describe('3D Model Viewer smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const BIMViewerPage = …                        (named const arrow)
    //   export function BIMViewerPage(…)                      (named function)
    //   export { BIMViewerPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+BIMViewerPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bBIMViewerPage\\b').test(src)
