...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Directory.tsx')

describe('Directory smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Directory = …                        (named const arrow)
    //   export function Directory(…)                      (named function)
    //   export { Directory } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Directory\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bDirectory\\b').test(src)
