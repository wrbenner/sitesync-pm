...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/walkthrough/index.tsx')

describe('Walk-Through Capture smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Walkthrough = …                        (named const arrow)
    //   export function Walkthrough(…)                      (named function)
    //   export { Walkthrough } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Walkthrough\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bWalkthrough\\b').test(src)
