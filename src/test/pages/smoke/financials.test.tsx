...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Financials.tsx')

describe('Financials smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Financials = …                        (named const arrow)
    //   export function Financials(…)                      (named function)
    //   export { Financials } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Financials\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bFinancials\\b').test(src)
