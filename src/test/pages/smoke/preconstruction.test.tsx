...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Preconstruction.tsx')

describe('Preconstruction smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Preconstruction = …                        (named const arrow)
    //   export function Preconstruction(…)                      (named function)
    //   export { Preconstruction } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Preconstruction\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPreconstruction\\b').test(src)
