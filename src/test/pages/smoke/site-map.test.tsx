...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/SiteMap.tsx')
...
describe('Site Map smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const SiteMap = …                        (named const arrow)
    //   export function SiteMap(…)                      (named function)
    //   export { SiteMap } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+SiteMap\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSiteMap\\b').test(src)
