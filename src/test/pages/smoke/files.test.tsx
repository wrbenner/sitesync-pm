...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/files/index.tsx')

describe('Files smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Files = …                        (named const arrow)
    //   export function Files(…)                      (named function)
    //   export { Files } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Files\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bFiles\\b').test(src)
