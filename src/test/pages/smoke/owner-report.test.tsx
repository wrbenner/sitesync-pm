...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/OwnerReportPage.tsx')

describe('Owner Report smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const OwnerReportPage = …                        (named const arrow)
    //   export function OwnerReportPage(…)                      (named function)
    //   export { OwnerReportPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+OwnerReportPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bOwnerReportPage\\b').test(src)
