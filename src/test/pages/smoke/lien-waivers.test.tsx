...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/LienWaivers.tsx')

describe('Lien Waivers smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const LienWaivers = …                        (named const arrow)
    //   export function LienWaivers(…)                      (named function)
    //   export { LienWaivers } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+LienWaivers\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bLienWaivers\\b').test(src)
