...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/rfis/RFIDetail.tsx')

describe('RFI Detail smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const RFIDetail = …                        (named const arrow)
    //   export function RFIDetail(…)                      (named function)
    //   export { RFIDetail } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+RFIDetail\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bRFIDetail\\b').test(src)
