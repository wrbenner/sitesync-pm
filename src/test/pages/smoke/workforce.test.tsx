...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Workforce.tsx')

describe('Workforce smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Workforce = …                        (named const arrow)
    //   export function Workforce(…)                      (named function)
    //   export { Workforce } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Workforce\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bWorkforce\\b').test(src)
