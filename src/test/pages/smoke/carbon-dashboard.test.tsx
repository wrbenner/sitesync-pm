...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/CarbonDashboard.tsx')

describe('Carbon Dashboard smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const CarbonDashboard = …                        (named const arrow)
    //   export function CarbonDashboard(…)                      (named function)
    //   export { CarbonDashboard } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+CarbonDashboard\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCarbonDashboard\\b').test(src)
