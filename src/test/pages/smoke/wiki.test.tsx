...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Wiki.tsx')
...
describe('Wiki smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Wiki = …                        (named const arrow)
    //   export function Wiki(…)                      (named function)
    //   export { Wiki } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Wiki\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bWiki\\b').test(src)
