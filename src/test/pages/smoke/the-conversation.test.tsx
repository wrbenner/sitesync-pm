...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/conversation/index.tsx')

describe('The Conversation smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Conversation = …                        (named const arrow)
    //   export function Conversation(…)                      (named function)
    //   export { Conversation } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Conversation\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bConversation\\b').test(src)
