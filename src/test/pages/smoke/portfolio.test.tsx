...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Portfolio.tsx')
...
describe('Portfolio smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Portfolio = …                        (named const arrow)
    //   export function Portfolio(…)                      (named function)
    //   export { Portfolio } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Portfolio\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPortfolio\\b').test(src)
