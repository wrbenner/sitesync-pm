...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/AdminOrgDetail.tsx')

describe('Admin — Organization Detail smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const AdminOrgDetail = …                        (named const arrow)
    //   export function AdminOrgDetail(…)                      (named function)
    //   export { AdminOrgDetail } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+AdminOrgDetail\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bAdminOrgDetail\\b').test(src)
