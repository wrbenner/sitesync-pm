...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/help/HelpArticle.tsx')

describe('Help Article smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const HelpArticle = …                        (named const arrow)
    //   export function HelpArticle(…)                      (named function)
    //   export { HelpArticle } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+HelpArticle\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bHelpArticle\\b').test(src)
