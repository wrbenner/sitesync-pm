...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/auth/VerifyPending.tsx')

describe('Verify Email smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const VerifyPending = …                        (named const arrow)
    //   export function VerifyPending(…)                      (named function)
    //   export { VerifyPending } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+VerifyPending\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bVerifyPending\\b').test(src)
