...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/cost-code-library/index.tsx')

describe('Cost Code Library smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const CostCodeLibrary = …                        (named const arrow)
    //   export function CostCodeLibrary(…)                      (named function)
    //   export { CostCodeLibrary } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+CostCodeLibrary\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCostCodeLibrary\\b').test(src)
