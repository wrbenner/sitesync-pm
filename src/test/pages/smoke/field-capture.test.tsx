...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/field-capture/index.tsx')

describe('Field Capture smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const FieldCapture = …                        (named const arrow)
    //   export function FieldCapture(…)                      (named function)
    //   export { FieldCapture } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+FieldCapture\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bFieldCapture\\b').test(src)
