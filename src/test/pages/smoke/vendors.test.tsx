...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Vendors.tsx')

describe('Vendors smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Vendors = …                        (named const arrow)
    //   export function Vendors(…)                      (named function)
    //   export { Vendors } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Vendors\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bVendors\\b').test(src)
