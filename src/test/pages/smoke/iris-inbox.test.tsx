...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/iris/IrisInboxPage.tsx')

describe('Iris Inbox smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const IrisInboxPage = …                        (named const arrow)
    //   export function IrisInboxPage(…)                      (named function)
    //   export { IrisInboxPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+IrisInboxPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bIrisInboxPage\\b').test(src)
