...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Estimating.tsx')

describe('Estimating smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Estimating = …                        (named const arrow)
    //   export function Estimating(…)                      (named function)
    //   export { Estimating } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Estimating\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bEstimating\\b').test(src)
