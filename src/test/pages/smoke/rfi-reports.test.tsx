...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/rfis/RFIReportsPage.tsx')

describe('RFI Reports smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const RFIReportsPage = …                        (named const arrow)
    //   export function RFIReportsPage(…)                      (named function)
    //   export { RFIReportsPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+RFIReportsPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bRFIReportsPage\\b').test(src)
