...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/UserProfile.tsx')

describe('User Profile smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const UserProfile = …                        (named const arrow)
    //   export function UserProfile(…)                      (named function)
    //   export { UserProfile } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+UserProfile\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bUserProfile\\b').test(src)
