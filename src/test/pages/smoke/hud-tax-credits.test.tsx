...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/compliance/HUDCompliancePage.tsx')

describe('HUD & Tax Credits smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const HUDCompliancePage = …                        (named const arrow)
    //   export function HUDCompliancePage(…)                      (named function)
    //   export { HUDCompliancePage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+HUDCompliancePage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bHUDCompliancePage\\b').test(src)
