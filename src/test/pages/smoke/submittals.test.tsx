...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/submittals/index.tsx')

describe('Submittals smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Submittals = …                        (named const arrow)
    //   export function Submittals(…)                      (named function)
    //   export { Submittals } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Submittals\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSubmittals\\b').test(src)
