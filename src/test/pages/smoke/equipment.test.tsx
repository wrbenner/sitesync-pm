...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Equipment.tsx')

describe('Equipment smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Equipment = …                        (named const arrow)
    //   export function Equipment(…)                      (named function)
    //   export { Equipment } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Equipment\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bEquipment\\b').test(src)
