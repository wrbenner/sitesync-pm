...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Contracts.tsx')
...
describe('Contracts smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Contracts = …                        (named const arrow)
    //   export function Contracts(…)                      (named function)
    //   export { Contracts } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Contracts\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bContracts\\b').test(src)
