...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Budget.tsx')
...
describe('Budget smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Budget = …                        (named const arrow)
    //   export function Budget(…)                      (named function)
    //   export { Budget } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Budget\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bBudget\\b').test(src)
