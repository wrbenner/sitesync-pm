...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Specifications.tsx')

describe('Specifications smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Specifications = …                        (named const arrow)
    //   export function Specifications(…)                      (named function)
    //   export { Specifications } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Specifications\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSpecifications\\b').test(src)
