...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/components/MagicLinkSubRoute.tsx')

describe('Sub Magic-Link Day Stream smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const MagicLinkSubRoute = …                        (named const arrow)
    //   export function MagicLinkSubRoute(…)                      (named function)
    //   export { MagicLinkSubRoute } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+MagicLinkSubRoute\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bMagicLinkSubRoute\\b').test(src)
