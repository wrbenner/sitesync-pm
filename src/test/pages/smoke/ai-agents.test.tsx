...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/AIAgents.tsx')
...
describe('AI Agents smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const AIAgents = …                        (named const arrow)
    //   export function AIAgents(…)                      (named function)
    //   export { AIAgents } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+AIAgents\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bAIAgents\\b').test(src)
