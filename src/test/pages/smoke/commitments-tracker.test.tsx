...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Commitments.tsx')

describe('Commitments Tracker smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Commitments = …                        (named const arrow)
    //   export function Commitments(…)                      (named function)
    //   export { Commitments } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Commitments\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bCommitments\\b').test(src)
