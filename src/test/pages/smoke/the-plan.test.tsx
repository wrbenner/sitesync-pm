...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/plan/index.tsx')

describe('The Plan smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Plan = …                        (named const arrow)
    //   export function Plan(…)                      (named function)
    //   export { Plan } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Plan\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPlan\\b').test(src)
