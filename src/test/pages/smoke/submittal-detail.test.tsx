...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/submittals/SubmittalDetailPage.tsx')

describe('Submittal Detail smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const SubmittalDetailPage = …                        (named const arrow)
    //   export function SubmittalDetailPage(…)                      (named function)
    //   export { SubmittalDetailPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+SubmittalDetailPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSubmittalDetailPage\\b').test(src)
