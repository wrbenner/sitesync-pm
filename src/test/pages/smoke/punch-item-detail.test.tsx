...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/punch-list/PunchItemDetailPage.tsx')

describe('Punch Item Detail smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const PunchItemDetailPage = …                        (named const arrow)
    //   export function PunchItemDetailPage(…)                      (named function)
    //   export { PunchItemDetailPage } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+PunchItemDetailPage\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPunchItemDetailPage\\b').test(src)
