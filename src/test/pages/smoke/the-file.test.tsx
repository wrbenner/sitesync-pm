...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/file/index.tsx')

describe('The File smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const File = …                        (named const arrow)
    //   export function File(…)                      (named function)
    //   export { File } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+File\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bFile\\b').test(src)
