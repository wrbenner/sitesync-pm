...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/day/index.tsx')

describe('The Day smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Day = …                        (named const arrow)
    //   export function Day(…)                      (named function)
    //   export { Day } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Day\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bDay\\b').test(src)
