...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/AuditTrail.tsx')

describe('Audit Trail smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const AuditTrail = …                        (named const arrow)
    //   export function AuditTrail(…)                      (named function)
    //   export { AuditTrail } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+AuditTrail\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bAuditTrail\\b').test(src)
