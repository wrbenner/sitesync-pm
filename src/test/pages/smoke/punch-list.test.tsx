...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/punch-list/index.tsx')

describe('Punch List smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const PunchList = …                        (named const arrow)
    //   export function PunchList(…)                      (named function)
    //   export { PunchList } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+PunchList\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bPunchList\\b').test(src)
