...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/Lookahead.tsx')

describe('Lookahead smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Lookahead = …                        (named const arrow)
    //   export function Lookahead(…)                      (named function)
    //   export { Lookahead } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Lookahead\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bLookahead\\b').test(src)
