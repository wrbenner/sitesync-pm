...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/daily-log/index.tsx')

describe('Daily Log smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const DailyLog = …                        (named const arrow)
    //   export function DailyLog(…)                      (named function)
    //   export { DailyLog } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+DailyLog\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bDailyLog\\b').test(src)
