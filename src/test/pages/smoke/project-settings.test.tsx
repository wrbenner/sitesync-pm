...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/admin/ProjectSettings.tsx')

describe('Project Settings smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const ProjectSettings = …                        (named const arrow)
    //   export function ProjectSettings(…)                      (named function)
    //   export { ProjectSettings } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+ProjectSettings\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bProjectSettings\\b').test(src)
