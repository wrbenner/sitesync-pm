...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/ProjectHealth.tsx')

describe('Project Health smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const ProjectHealth = …                        (named const arrow)
    //   export function ProjectHealth(…)                      (named function)
    //   export { ProjectHealth } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+ProjectHealth\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bProjectHealth\\b').test(src)
