...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/schedule/index.tsx')
...
describe('Schedule smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Schedule = …                        (named const arrow)
    //   export function Schedule(…)                      (named function)
    //   export { Schedule } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Schedule\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bSchedule\\b').test(src)
