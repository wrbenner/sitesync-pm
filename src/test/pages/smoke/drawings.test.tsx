...
const PAGE_FILE = path.resolve(__dirname, '..', '..', '..', '..', 'src/pages/drawings/index.tsx')

describe('Drawings smoke', () => {
  ...
  it('declares a React-component export', () => {
    ...
    //   export const Drawings = …                        (named const arrow)
    //   export function Drawings(…)                      (named function)
    //   export { Drawings } from '…'                     (re-export)
    const hasDefault = /export\s+default\s+/.test(src)
    const hasNamed = new RegExp(
      'export\\s+(?:const|function|async\\s+function)\\s+Drawings\\b',
    ).test(src)
    const hasReexport = new RegExp('export\\s*\\{[^}]*\\bDrawings\\b').test(src)
