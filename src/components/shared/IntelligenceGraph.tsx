...
export function IntelligenceGraph({
  ...
  useEffect(() => {
    ...
    const simNodes: SimNode[] = inputNodes.map((n, i) => {
      ...
      return {
        ...n,
        x: existing?.x ?? Math.cos(angle) * spread + Math.cos(angle + Math.PI / 2) * 20,
        y: existing?.y ?? Math.sin(angle) * spread + Math.sin(angle + Math.PI / 2) * 20,
        vx: existing?.vx ?? 0,
        vy: existing?.vy ?? 0,
        fx: 0,
        fy: 0,
  ...
  const physicsStep = useCallback(() => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (dist < 1) { dist = 1; dx = 0.5; dy = 0.5 }
