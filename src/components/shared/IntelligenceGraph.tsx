...
export function IntelligenceGraph({
  ...
  useEffect(() => {
    ...
    const simNodes: SimNode[] = inputNodes.map((n, i) => {
      ...
      return {
        ...n,
        x: existing?.x ?? Math.cos(angle) * spread + (Math.random() - 0.5) * 40, // immune-ok — force-directed graph physics
        y: existing?.y ?? Math.sin(angle) * spread + (Math.random() - 0.5) * 40, // immune-ok — force-directed graph physics
        vx: existing?.vx ?? 0,
        vy: existing?.vy ?? 0,
        fx: 0,
        fy: 0,
  ...
  const physicsStep = useCallback(() => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (dist < 1) { dist = 1; dx = Math.random() - 0.5; dy = Math.random() - 0.5 } // immune-ok — force-directed graph physics jitter
