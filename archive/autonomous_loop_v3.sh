...
PROMPT_TIMEOUT="${PROMPT_TIMEOUT:-1200}"
...
evolve_prompt_strategy() {
    ...
    for verify_file in "${cycle_dir}"/verify_*.json; do
        ...
        rate="${rate:-0}"; fixed="${fixed:-0}"; total="${total:-0}"
...
verify_ui_polish() {
    ...
    # Write polish report
    issues="${issues:-0}"; hc_count="${hc_count:-0}"; px_count="${px_count:-0}"; log_count="${log_count:-0}"
    jq -n \
        --argjson issues "$issues" \
        --argjson cycle "${CYCLE:-0}" \
        --arg warnings "$(printf '%b' "$warnings")" \
        --argjson hardcoded_colors "$hc_count" \
        --argjson hardcoded_px "$px_count" \
        --argjson console_logs "$log_count" \
        '{
            cycle: $cycle,
...
verify_changes() {
    ...
    fixed_count="${fixed_count:-0}"; partial_count="${partial_count:-0}"; unfixed_count="${unfixed_count:-0}"
    [ -z "$verifications" ] || ! echo "$verifications" | jq '.' >/dev/null 2>&1 && verifications="[]"
    jq -n \
        --argjson verifications "$verifications" \
        --argjson files_changed "${files_changed_count:-0}" \
